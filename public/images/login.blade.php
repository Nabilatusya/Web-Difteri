        ...
        <div class="max-w-6xl w-full bg-white rounded-[24px] shadow-lg overflow-hidden flex flex-col md:flex-row h-full">
            ...
            <div class="w-full md:w-1/2 p-20 flex flex-col justify-center">
                ...
                <form method="POST" action="{{ route('login') }}">
                    @csrf
                    <div class="mb-4">
                        <label for="email" class="block text-sm font-medium text-[#1B262C]">{{ __('Email Address') }}</label>
                    ...
                    <div class="mb-4">
                        <label for="password" class="block text-sm font-medium text-[#1B262C]">{{ __('Password') }}</label>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-sm text-[#3148A8] hover:underline">{{ __('Forgot Your Password?') }}</a>
                        @endif
                    </div>
                    <button type="submit" class="w-full bg-[#3148A8] text-white rounded-lg py-3 hover:bg-[#537FE7]">{{ __('Masuk') }}</button>
                </form>
            </div>
        </div>
    </div>
